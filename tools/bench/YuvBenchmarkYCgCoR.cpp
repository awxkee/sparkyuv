...
void SparkyuvYCgCoR444ToRGBA8(benchmark::State &state) {
  ...
  sparkyuv::RGBA8ToYCgCoR420P8(rgbaData.data(), rgbaStride, inWidth, inHeight,
...
void SparkyuvYCgCoR422ToRGBA8(benchmark::State &state) {
  ...
  rgbaData.resize(rgbaStride * inHeight);
  sparkyuv::RGBToRGBA(inSrcData.data(), inWidth * sizeof(uint8_t) * 3, rgbaData.data(), rgbaStride, inWidth, inHeight);
  sparkyuv::RGBA8ToYCgCoR422P8(rgbaData.data(), rgbaStride, inWidth, inHeight,
                              yPlane.data(), yPlaneStride,
                              uPlane.data(), uvPlaneStride,
                              vPlane.data(), uvPlaneStride, sparkyuv::YCGCO_RE);
  for (auto _ : state) {
    sparkyuv::YCgCoR422P8ToRGBA8(rgbaData.data(), rgbaStride, inWidth, inHeight,
                                yPlane.data(), yPlaneStride,
                                uPlane.data(), uvPlaneStride,
                                vPlane.data(), uvPlaneStride, sparkyuv::YCGCO_RE);
  }
...
void SparkyuvRGBA8ToYCgCoR444(benchmark::State &state) {
  ...
  sparkyuv::RGBToRGBA(inSrcData.data(), inWidth * sizeof(uint8_t) * 3, rgbaData.data(), rgbaStride, inWidth, inHeight);
  for (auto _ : state) {
    sparkyuv::RGBA8ToYCgCoR444P8(rgbaData.data(), rgbaStride, inWidth, inHeight,
                                yPlane.data(), yPlaneStride,
                                uPlane.data(), uvPlaneStride,
                                vPlane.data(), uvPlaneStride, sparkyuv::YCGCO_RE);
  }
