...
HWY_FAST_MATH_INLINE V
Exp2f(const DF dhf, V x) {
  const Half<decltype(dhf)> dh;
  const Rebind<float32_t, decltype(dh)> df;
  const auto lo = Exp2f(df, PromoteLowerTo(df, x));
  const auto hi = Exp2f(df, PromoteUpperTo(df, x));
  return Combine(dhf, DemoteTo(dhf, hi), DemoteTo(dhf, lo));
}
...
Log10f(const DF df, V x) {
  const auto baseScalar = Set(df, hwy::F16FromF32(10.0f));
...
template<class DF, class V, HWY_IF_F16_D(DF)>
HWY_FAST_MATH_INLINE V Pow(const DF dhf, V val, V n) {
  const Half<decltype(dhf)> dh;
  const Rebind<float32_t, decltype(dh)> df;
  const auto lo = Exp(df, Mul(PromoteLowerTo(df, n), Lognf(df, PromoteLowerTo(df, val))));
  const auto hi = Exp(df, Mul(PromoteUpperTo(df, n), Lognf(df, PromoteUpperTo(df, val))));
  return Combine(dhf, DemoteTo(dhf, hi), DemoteTo(dhf, lo));
}
