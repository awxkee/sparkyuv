...
template<class D, class V, HWY_IF_F32_D(D)>
HWY_INLINE V
Log10(const D d, V x) {
  using T = TFromD<D>;
...
HWY_INLINE V
Log10(const D dhf, V x) {
  const Half<decltype(dhf)> dh;
  const Rebind<float32_t, decltype(dh)> df;
  const auto lo = Log10(df, PromoteLowerTo(df, x));
  const auto hi = Log10(df, PromoteUpperTo(df, x));
