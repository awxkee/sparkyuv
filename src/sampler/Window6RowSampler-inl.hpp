...
#endif
      for (int c = 0; c < components; ++c) {
        dst[px + c] = static_cast<uint8_t>(std::clamp(std::roundf(rgb[c] * weightSum), 0.0f, maxColors));
      }
    }
  }
...
#endif
      for (int c = 0; c < components; ++c) {
        dst[px + c] = static_cast<uint8_t>(std::clamp(std::roundf(rgb[c] * invWeightScale), 0.0f, maxColors));
      }
    }
  }
