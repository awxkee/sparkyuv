...
void NV21ToPixel8(uint8_t *SPARKYUV_RESTRICT dst,
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (; x + lanes < width; x += lanes) {
      ...
      const VU16 g = ShiftRight<6>(Max(SaturatedSub(light, SaturatedAdd(Mul(ivGCoeff1, cr), Mul(ivGCoeff2, cb))), vZero));
...
void Pixel8ToNV21HWY(const uint8_t *SPARKYUV_RESTRICT src, const uint32_t srcStride,
  ...
  for (uint32_t y = 0; y < height; ++y) {
    ...
    for (; x < width; x += 2) {
      ...
      switch (PixelType) {
        case PIXEL_RGB:
        case PIXEL_RGBA:r1 = static_cast<int>(mNextSrc[0]);
          g1 = static_cast<int>(mNextSrc[1]);
          b1 = static_cast<int>(mNextSrc[2]);
          break;
        case PIXEL_BGRA:
        case PIXEL_BGR:r1 = static_cast<int>(mNextSrc[2]);
          g1 = static_cast<int>(mNextSrc[1]);
          b1 = static_cast<int>(mNextSrc[0]);
          break;
        case PIXEL_ARGB:r1 = static_cast<int>(mNextSrc[1]);
          g1 = static_cast<int>(mNextSrc[2]);
          b1 = static_cast<int>(mNextSrc[3]);
          break;
        case PIXEL_ABGR:r1 = static_cast<int>(mNextSrc[3]);
          g1 = static_cast<int>(mNextSrc[2]);
          b1 = static_cast<int>(mNextSrc[1]);
          break;
      }
      ...
      if (x + 1 < width) {
        ...
        if (!(y & 1)) {
          switch (PixelType) {
            case PIXEL_RGB:
            case PIXEL_RGBA:r3 = static_cast<int>(mNextSrc[0]);
              g3 = static_cast<int>(mNextSrc[1]);
              b3 = static_cast<int>(mNextSrc[2]);
              break;
            case PIXEL_BGRA:
            case PIXEL_BGR:r3 = static_cast<int>(mNextSrc[2]);
              g3 = static_cast<int>(mNextSrc[1]);
              b3 = static_cast<int>(mNextSrc[0]);
              break;
            case PIXEL_ARGB:r3 = static_cast<int>(mNextSrc[1]);
              g3 = static_cast<int>(mNextSrc[2]);
              b3 = static_cast<int>(mNextSrc[3]);
              break;
            case PIXEL_ABGR:r3 = static_cast<int>(mNextSrc[3]);
              g3 = static_cast<int>(mNextSrc[2]);
              b3 = static_cast<int>(mNextSrc[1]);
              break;
          }
